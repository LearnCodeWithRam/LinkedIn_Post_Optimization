"""
Viral Post Analysis Cache Service

Manages caching of AI analysis results for viral posts using JSON file storage.
Separate from regular post analysis cache.
"""
...
class ViralPostAnalysisCacheService:
    """
    Service for caching viral post analysis results to JSON file.
    """
    
    # Cache configuration
    CACHE_DIR = Path(__file__).parent / 'cache_viral_post'
    ...
    def _ensure_cache_dir(self):
        ...
        try:
            ...
            logger.info(f"Viral post cache directory ready: {self.CACHE_DIR}")
        except Exception as e:
            logger.error(f"Failed to create viral post cache directory: {e}")
    ...
    def _load_cache(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if cache_data.get('version') != self.CACHE_VERSION:
                logger.warning(f"Viral post cache version mismatch. Clearing cache.")
                return {
                    'version': self.CACHE_VERSION,
                    'analyses': {}
                }
            ...
            logger.error(f"Viral post cache file corrupted: {e}. Creating new cache.")
            return {
                'version': self.CACHE_VERSION,
                'analyses': {}
            }
        except Exception as e:
            logger.error(f"Error loading viral post cache: {e}")
            return {
                'version': self.CACHE_VERSION,
                'analyses': {}
            }
    ...
    def _save_cache(self, cache_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.debug(f"Viral post cache saved: {len(cache_data.get('analyses', {}))} entries")
        
        except Exception as e:
            logger.error(f"Error saving viral post cache: {e}")
    ...
    def _clean_expired_entries(self, analyses: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if removed_count > 0:
            logger.info(f"Removed {removed_count} expired viral post cache entries")
    ...
    def _trim_cache(self, analyses: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if removed_count > 0:
            logger.info(f"Trimmed {removed_count} oldest viral post cache entries")
        
        return trimmed
    
    def get_cached_analysis(self, post_id: str) -> Optional[Dict[str, Any]]:
        """
        Get cached analysis for a viral post ID.
        
        Args:
            post_id: Unique identifier for the viral post
        ...
        try:
            ...
            if not cached_entry:
                logger.debug(f"No viral post cache found for post_id: {post_id}")
                return None
            ...
            if expires_at and expires_at <= now:
                logger.debug(f"Viral post cache expired for post_id: {post_id}")
                self.remove_cached_analysis(post_id)
                return None
            
            logger.info(f"✓ Viral post cache hit for post_id: {post_id}")
            return cached_entry.get('analysis_data')
        
        except Exception as e:
            logger.error(f"Error getting cached viral post analysis: {e}")
            return None
    ...
    def set_cached_analysis(
        ...
        """
        Save viral post analysis to cache.
        
        Args:
            post_id: Unique identifier for the viral post
            post_content: Content of the viral post (for reference)
        ...
        try:
            ...
            logger.info(f"✓ Cached viral post analysis for post_id: {post_id} (expires: {expires_at.date()})")
        
        except Exception as e:
            logger.error(f"Error caching viral post analysis: {e}")
    
    def remove_cached_analysis(self, post_id: str):
        """
        Remove cached analysis for a specific viral post.
        ...
        try:
            ...
            if post_id in cache_data.get('analyses', {}):
                ...
                self._save_cache(cache_data)
                logger.info(f"Removed viral post cache for post_id: {post_id}")
        
        except Exception as e:
            logger.error(f"Error removing cached viral post analysis: {e}")
    
    def clear_all_cache(self):
        """Clear all cached viral post analyses."""
        try:
            cache_data = {
                'version': self.CACHE_VERSION,
                'analyses': {}
            ...
            self._save_cache(cache_data)
            logger.info("Cleared all cached viral post analyses")
        
        except Exception as e:
            logger.error(f"Error clearing viral post cache: {e}")
    
    def get_cache_stats(self) -> Dict[str, Any]:
        """
        Get viral post cache statistics.
        ...
        try:
            ...
            logger.error(f"Error getting viral post cache stats: {e}")
            return {
                'total_entries': 0,
                'cache_size_bytes': 0,
                'cache_size_kb': 0,
    ...
    def has_cached_analysis(self, post_id: str) -> bool:
        """
        Check if viral post has valid cached analysis.
...
# Singleton instance
viral_post_analysis_cache_service = ViralPostAnalysisCacheService()
