    ...
    useEffect(() => {
        ...
        ; (async () => {
            try {
                ...
                const url = `${apiUrl}/api/v1/accounts/google/signin/callback/?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50 flex items-center justify-center">
            <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
                {error ? (
                    ...
                    <>
                        <div className="mb-4">
                            <svg className="animate-spin mx-auto h-12 w-12 text-[#4285F4]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                        </div>
                        <h2 className="text-xl font-bold text-gray-900 mb-2">Completing Sign-in</h2>
                        <p className="text-gray-600">Processing your Google credentials...</p>
                    </>
                )}
            </div>
        </div>
